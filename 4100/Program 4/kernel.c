...
void enqueue_priority(pcbq_t *highPri, pcbq_t *medPri, pcbq_t *lowPri, pcb_t *pcb); // New Program 4
pcb_t *dequeue(pcbq_t *q);
pcb_t *dequeue_priority(pcbq_t *highPri, pcbq_t *medPri, pcbq_t *lowPri); // New Program 4
...
int create_process(uint32_t code_address, uint32_t priority)   
{
    ...
    *st = 0;
    ...
    temp->pid = pid;
    pid++;
    enqueue_priority(highPri, medPri, lowPri, temp);
...
void enqueue(pcbq_t *q, pcb_t *pcb)   
{
    ...
    pcb_t *temp, *current;
    temp = (pcb_t*)kmalloc(sizeof(pcb_t));
    ...
    else
    {
        ...
        {
            current = (pcb_t*)current->next;
        }
        current->next = (struct pcb_t*)temp;
        q->tail=temp;
...
void enqueue_priority(pcbq_t *highPri, pcbq_t *medPri, pcbq_t *lowPri, pcb_t *pcb)
{
    if(pcb->priority == 3)
    {
        enqueue(highPri, pcb);
...
pcb_t *dequeue_priority(pcbq_t *highPri, pcbq_t *medPri, pcbq_t *lowPri)
{
    pcb_t *temp;
    if(highPri->head != NULL)
    {
        temp = dequeue(highPri);
    }
    else if(medPri->head != NULL && highPri->head == NULL)
    {
        temp = dequeue(medPri);
    }
    else if(lowPri->head != NULL && medPri->head == NULL)
    {
        temp = dequeue(lowPri);
    }
